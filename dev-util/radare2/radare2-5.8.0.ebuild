...
PATCHES=(
	"${FILESDIR}/${PN}-5.8.0-vector35.patch"
	"${FILESDIR}/${PN}-5.8.0-capstone-fix.patch"
)
...
src_prepare() {
	...
	mv "${WORKDIR}/vector35-arch-arm64-${ARM64_COMMIT}" libr/arch/p/arm/v35arm64/arch-arm64 || die
	mv "${WORKDIR}/vector35-arch-armv7-${ARMV7_COMMIT}" libr/arch/p/arm/v35arm64/arch-armv7 || die
...
src_configure() {
	...
	econf \
		...
		--with-sysxxhash \
		--with-syszip
		# Doesn't build with OpenSSL: https://github.com/radareorg/radare2/issues/21193
