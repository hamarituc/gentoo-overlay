...
SRC_URI="
	...
	https://www-user.tu-chemnitz.de/~hamari/distfiles/${P}-dependencies-${GRADLE_DEP_VER}.tar.gz
	https://github.com/pxb1988/dex2jar/releases/download/v2.1/dex2jar-2.1.zip
	https://github.com/NationalSecurityAgency/ghidra-data/raw/Ghidra_${PV}/lib/java-sarif-2.1-modified.jar
...
src_unpack() {
	...
	cp "${DISTDIR}/AXMLPrinter2.jar" ./flatRepo  || die "(5) cp failed"
	...
	mkdir -p "${WORKDIR}/ghidra.bin/Ghidra/Features/BSim/" || die "(8) mkdir failed"
	cp "${DISTDIR}/postgresql-15.3.tar.gz" "${WORKDIR}/ghidra.bin/Ghidra/Features/BSim/" || die "(9) cp failed"

	PLUGIN_DEP_PATH="ghidra.bin/GhidraBuild/EclipsePlugins/GhidraDev/buildDependencies"
	mkdir -p "${WORKDIR}/${PLUGIN_DEP_PATH}/" || die "(10) mkdir failed"
	cp "${DISTDIR}/PyDev-6.3.1.zip" "${WORKDIR}/${PLUGIN_DEP_PATH}/PyDev 6.3.1.zip" || die "(11) cp failed"
	cp "${DISTDIR}/cdt-8.6.0.zip"   "${WORKDIR}/${PLUGIN_DEP_PATH}/" || die "(12) cp failed"
...
src_prepare() {
	mkdir -p ".gradle/init.d"                       || die "(13) mkdir failed"
	cp "${FILESDIR}"/repos.gradle .gradle/init.d    || die "(14) cp failed"
	sed -i "s|S_DIR|${S}|g" .gradle/init.d/repos.gradle || die "(15) sed failed"
	#remove build date so we can unpack dist.zip later
	sed -i "s|_\${rootProject.BUILD_DATE_SHORT}||g" gradle/root/distribution.gradle || die "(16) sed failed"
...
src_install() {
	...
	find "${ED}"/usr/share/ghidra -type f -name '*.zip' -delete
